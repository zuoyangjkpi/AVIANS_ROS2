...
class NMPCTrackerNode(Node):
    ...
    def _init_publishers(self):
        ...
        # Velocity setpoints are generated by the waypoint controller; NMPC should not own that topic.
    ...
    def control_loop_callback(self):
        ...
        if self.person_detected and current_time - self.last_person_detection_time > self.person_timeout:
            self.person_detected = False
            self.controller.person_detected = False
    ...
    def _perform_tracking(self):
        """Execute NMPC optimization and send tracking commands"""
        try:
            ...
            self.get_logger().error(f"Tracking step failed: {exc}")
            self._enter_lost_hold()
    ...
    def _send_lost_hold_command(self):
        ...
        if self.last_tracking_yaw is None:
            self.last_tracking_yaw = self._get_current_yaw()
        self._publish_waypoint(self.last_known_position)
        self._publish_attitude(self.last_tracking_yaw)
