...
namespace target_tracker_distributed_kf {
        ...
        public:
            ...
            void print(std::ostream &stream) const{
                ...
                for(auto it=begin(); it != end(); ++it){
                    stream << "\t" << it->stamp.seconds() << "\t" << it->robot << "\t" << it->measurements.size() << "\t\t" << it->frame_id << "\t" << it->isSelfRobot << it->state.format(NoEndLineOnRowSep) << std::endl;
                }
            }
    };
    ...
    class DistributedKF3D : public rclcpp::Node {
        ...
        private:
            ...
            PoseWithCovarianceStamped selfMsg_;
            TwistWithCovarianceStamped selfVelMsg_;    
            ...
            void predictAndPublish(uav_msgs::msg::UAVPose::ConstSharedPtr pose);
